    ...
    const isDoubleChar = (s) => {
        ...
        refreshInfo = () => {
            $('info').innerHTML = '<i>' + fullLine + '</i><br />' +
                '명령어안내(C) ' + (pwd !== 'top' || currentUser !== null ? '초기메뉴(T) ' : '로그인(L) ') + (currentRouter.type === 'board' ? '글쓰기(W) 다음(Enter,F) 이전(B) ' : '') + (pwd === 'top' ? '직접이동(GO) ' : '') + '이동(P,M) 종료(X)<br />';
            setPrompt();
        },
        isNoTitle = (newpwd) => {
                ...
                loadWelcome = async () => {
                    ...
                    loadBoard = async (newpwd, page) => {
                        ...
                        loadFeed = async (feedid, page = 1) => {
                                await fetch('/api/board/' + pwd + '/' + feedid + '/' + page).then(resp => resp.text()).then(d => {
                                    try {
